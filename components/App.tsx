...
// Components
import { Sidebar, PageView } from './layout/Sidebar';
import { Dashboard } from './dashboard/Dashboard';
import { StoneCard } from './inventory/StoneCard';
import { ClientView } from './clients/ClientView';
import { ToastContainer } from './ui/Toast';
import { InventoryFilters } from './inventory/InventoryFilters';
import { CatalogView } from './catalog/CatalogView';
import { BatchModal } from './inventory/BatchModal';
import { TypologyModal } from './catalog/TypologyModal';
import { DelegateModal } from './offers/DelegateModal';
import { OfferModal } from './offers/OfferModal';
import { DirectLinkModal } from './offers/DirectLinkModal';
import { TransactionDetailsModal } from './offers/TransactionDetailsModal';
import { AnalyticsDetailView } from './analytics/AnalyticsDetailView';
import { SellerInventoryModal } from './inventory/SellerInventoryModal';
import { IndustryInventoryModal } from './inventory/IndustryInventoryModal';
import { InterestThermometerView } from './analytics/InterestThermometerView';
import { NotificationDropdown } from './layout/NotificationDropdown';
import { CRMView } from './clients/CRMView';
...
const AppContent = () => {
   ...
   const kpi = useMemo(() => {
      ...
      return {
         ...
         soldRevenue,
         totalProfit,
         reservedLinks: pipelineData.filter(item => item.offer.status === 'reserved').length,
   ...
   const handleCreateOffer = (offer: OfferLink) => {
      const withTenant = offer.tenantId ? offer : { ...offer, tenantId: currentTenantId };
      setOffers(prev => [withTenant, ...prev]);
   ...
   const handleSaveClient = (client: Client) => {
      setClients(prev => {
         ...
         const normalized = client.tenantId ? client : { ...client, tenantId: currentTenantId };
         if (exists) return prev.map(c => c.id === client.id ? normalized : c);
   ...
   const renderInventory = () => {
      const filteredStones = stones.filter(s => {
         ...
         // Três estados: disponível, reservado ou esgotado (vendido/zerado)
               ...
               <div className="flex items-center gap-4">
                  {currentUserRole === 'industry_admin' && (
                     <div className="bg-slate-100 p-1 rounded-sm flex">
                        ...
                        >
                           <LayoutGrid className="w-4 h-4 mr-2" /> {t('inv.switch.lots')}
                        </button>
                        <button
                        ...
                        >
                           <List className="w-4 h-4 mr-2" /> {t('inv.switch.catalog')}
                        </button>
                     </div>
                  )}
                  ...
                  {currentUserRole === 'industry_admin' && inventoryMode === 'catalog' && (
                     <button onClick={() => setActiveModal({ type: 'typology', data: null })} className="px-6 py-3 bg-[#121212] text-white text-xs font-bold uppercase tracking-widest flex items-center rounded-sm hover:bg-[#C2410C] transition-colors shadow-lg">
                        <Plus className="w-4 h-4 mr-2" /> {t('inv.add_typology')}
                     </button>
                  )}
               </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                     {filteredStones.map((s, i) => (
                        <StoneCard
                           ...
                           index={i}
                           inventorySnapshot={InventoryService.computeProgressSnapshot(s, delegations, offers)}
                           onClick={() => setActiveModal({ type: currentUserRole === 'industry_admin' ? 'industry_inv' : 'seller_inv', data: currentUserRole === 'industry_admin' ? s : { stone: s, delegation: delegations.find(d => d.stoneId === s.id && d.sellerId === currentSellerId) } })}
