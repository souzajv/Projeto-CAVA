      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="mb-3">
          <div className="flex justify-between items-start">
            <h3 className="font-semibold text-slate-900 text-lg leading-tight">{stone.typology.name}</h3>
          </div>
        ...
        {role === 'industry_admin' && (
          <div className="mb-5 space-y-2">
            <div className="flex justify-between text-xs font-medium">
              <span className="text-emerald-700">Avail: {stone.quantity.available}</span>
              <span className="text-amber-700">Reserved: {stone.quantity.reserved}</span>
              <span className="text-slate-400">Sold: {stone.quantity.sold}</span>
            </div>
            <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden flex">
              <div style={{ width: `${soldPct}%` }} className="bg-slate-300" />
              <div style={{ width: `${reservedPct}%` }} className="bg-amber-400" />
              <div style={{ width: `${availPct}%` }} className="bg-emerald-500" />
            </div>
          </div>
        )}
          ...
          {role === 'industry_admin' ? (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 mt-3">
                <button 
                  onClick={() => isAvailable && onDirectLink?.(stone)}
                ...
                <button 
                  onClick={() => isAvailable && onDelegate?.(stone)}
                  disabled={!isAvailable}
            ...
            <div className="space-y-3">
              {delegation ? (
                <>
                  <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 space-y-1">
                    <div className="flex justify-between items-center text-xs text-blue-800">
                       <span className="font-semibold flex items-center"><Layers className="w-3 h-3 mr-1"/> Your Stock</span>
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={() => onViewHistory?.(delegation)}
                      className="flex items-center justify-center px-3 py-2 bg-white border border-slate-200 text-slate-500 rounded-lg text-xs font-medium hover:bg-slate-50 hover:text-slate-700 transition-colors"
                    ...
                    <button 
                      onClick={() => onCreateOffer?.(delegation)}
                ...
                <div className="text-center text-sm text-slate-400 italic py-2">
                  Not currently in your showroom
                </div>
              )}
            </div>
          )}
