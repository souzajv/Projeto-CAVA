    ...
    return (
      <div className="relative group flex-1">
        <button 
          ...
          className={`relative w-full p-8 rounded-sm transition-all duration-500 text-left overflow-hidden min-h-[14rem] flex flex-col justify-between border ${
            isActive 
            ? 'border-transparent shadow-2xl scale-[1.02] z-20' 
            : 'bg-white border-slate-100 hover:border-[#121212] hover:shadow-xl z-10'
          }`}
        >
          {isActive && (
          ...
          <div className="flex justify-between items-start relative z-10">
            <Icon className={`w-6 h-6 transition-colors duration-300 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-[#C2410C]'}`} />
            <span className={`text-5xl font-serif transition-colors duration-300 ${isActive ? 'text-white' : 'text-[#121212] group-hover:text-white'}`}>
              {count}
            </span>
          </div>
          
          <div className="relative z-10 flex flex-col justify-end">
            <p className={`text-[10px] font-bold uppercase tracking-[0.3em] transition-colors duration-300 mb-1 ${isActive ? 'text-white/80' : 'text-slate-400 group-hover:text-[#C2410C]'}`}>
              {label}
            </p>
            ...
            <div className={`grid transition-[grid-template-rows] duration-500 ease-in-out ${isActive ? 'grid-rows-[1fr]' : 'grid-rows-[0fr] group-hover:grid-rows-[1fr]'}`}>
               <div className="overflow-hidden">
                  <p className={`text-[11px] leading-relaxed pt-3 font-medium transition-all duration-500 ${
                    isActive 
                      ? 'text-white/90 opacity-100' 
                      : 'text-slate-400 opacity-0 group-hover:opacity-100 group-hover:text-white/80'
                  }`}>
                    {description}
                  </p>
               </div>
