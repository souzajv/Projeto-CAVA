...
export const DelegateModal: React.FC<DelegateModalProps> = ({ stone, sellers, onClose, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 transition-all duration-300">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200 ring-1 ring-slate-900/5">
        
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 tracking-tight">Delegate Inventory</h2>
            <p className="text-sm text-slate-500">Assign quantity to sales partner</p>
          </div>
          <button 
            onClick={onClose} 
              ...
              <div className="flex flex-col justify-center">
                ...
                <div className="flex items-center text-xs text-slate-500 mt-1 space-x-2">
                  ...
                  <span className="text-emerald-600 font-medium">
                    Available: {stone.quantity.available} {stone.quantity.unit}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <label className="text-sm font-medium text-slate-900 flex items-center justify-between">
                  <span>Quantity to Delegate</span>
                  <span className="text-xs text-slate-500">Max: {stone.quantity.available}</span>
                </label>
