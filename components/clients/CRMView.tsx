...
export const CRMView: React.FC<CRMViewProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    const reservedOffers = relevantOffers.filter(o => o.status === 'reserved');

    return {
      total: visibleClients.length,
      activeLinks: activeOffers.length,
      reservedLinks: reservedOffers.length,
    ...
    <div className="space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="p-8 bg-white border border-slate-100 shadow-sm rounded-sm">
          ...
          <p className="mt-2 text-[11px] text-slate-500 font-semibold">{t('dash.kpi.reserved_links')}: {stats.reservedLinks}</p>
        </div>
